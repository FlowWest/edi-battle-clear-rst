...
# The flow data will from CDEC
# Temperature data will be from the UBC temperature loggger
...
# Change brood year when needed
l.names.fl.a <- c("jdate", "BY21 Late-Fall minimum FL", "BY21 Late-fall maximum FL")
l.names.fl.b <- c("jdate", "BY22 Late-Fall minimum FL", "BY22 Late-fall maximum FL")
w.names.fl.a <- c("jdate", "BY21 Winter minimum FL", "BY21 Winter maximum FL")
w.names.fl.b <- c("jdate", "BY22 Winter minimum FL", "BY22 Winter maximum FL")
s.names.fl <- c("jdate", "BY21 Spring minimum FL", "BY21 Spring maximum FL")
r.names.fla <- c("jdate", "BY21 RBT FL", "BY21 RBT maximum FL")
r.names.flb <- c("jdate", "BY22 RBT minimum FL", "BY22 RBT maximum FL")

# Load needed data sets
# Load data sets
cat <- read.csv("BOR UBC.csv")
sid <-  read.csv("UBC SampleID.csv")
...
sid$SDate <- as.Date(as.POSIXct(sid$Date, format = "%m/%d/%Y"))
...
cat1.fl$Date <- as.Date(as.POSIXct(cat1.fl$Date, format = "%m/%d/%Y"))

# Filter for brood year and race and get just the date and FL
rbt.1 <- cat1.fl %>% filter(BroodYear == by1, OrganismCode == "RBT") %>%
              select(Date, ForkLength)
...
# Get just the date from sid
sid1 <- sid %>% select(Date, jdate)

sid1$Date <- as.Date(as.POSIXct(sid1$Date, format = "%m/%d/%Y"))
rbt.2$Date <- as.Date(as.POSIXct(rbt.2$Date, format = "%m/%d/%Y"))

# Merge sid and fl data frames
fl.wa <- merge(sid1, wcs.1, all=T)
fl.wb <- merge(sid1, wcs.2, all=T)
fl.s <- merge(sid1, scs, all=T)
fl.lfa <- merge(sid1, lfcs.1, all=T)
fl.lfb <- merge(sid1, lfcs.2, all=T)
rbt.fla <- merge(sid1, rbt.1, all=T)
rbt.flb <- merge(sid1, rbt.2, all=T)
...
fl.wa <- fl.wa %>% select(SampleDate, minimum, maximum)
fl.wb <- fl.wb %>% select(SampleDate, minimum, maximum)
fl.s <- fl.s %>% select(SampleDate, minimum, maximum)
fl.lfa <- fl.lfa %>% select(SampleDate, minimum, maximum)
fl.lfb <- fl.lfb %>% select(SampleDate, minimum, maximum)
rbt.fla <- rbt.fla %>% select(SampleDate, minimum, maximum)
rbt.flb <- rbt.flb %>% select(SampleDate, minimum, maximum)
...
# merge again with sid1
fl.wa <- merge(sid1, fl.wa)
fl.wb <- merge(sid1, fl.wb)
fl.s <- merge(sid1, fl.s)
fl.lfa <- merge(sid1, fl.lfa)
fl.lfb <- merge(sid1, fl.lfb)
rbt.fla <- merge(sid1, rbt.fla)
rbt.flb <- merge(sid1, rbt.flb)
...
fork.p <- cbind(passage, fork)

# Bind passage and fork data frames
fork.p <- cbind(passage, fork)

# Rename second date
names(fork.p)[10] <- "Date.1"

# Drop the extra date
fork.p <- fork.p %>% select(-"Date.1")

fork.p$Date <- as.Date(as.POSIXct(fork.p$Date, format = "%m/%d/%Y"))

################ UBC hourly water temperatures ################
# Laod needed data 
temp <- read.csv("UBC Temps.csv")
flow <- read.csv("BATTLE CREEK (flow).csv")

#  Give the columns better names
names(temp)[1] <- 'Date'
names(temp)[3] <- 'C'
names(temp)[4] <- 'Temperature'

names(flow)[1] <- 'Date'
names(flow)[2] <- 'nothing'
names(flow)[3] <- 'cfs'

# loose the Time and C columns
temp <- temp %>% select(-c(Time, Temperature))
...
flow$Date <- as.Date(as.POSIXct(flow$Date, format = "%m/%d/%Y"))
...
# Calculate daily mean daily temp at UBC
mdt <- temp %>% group_by(Date) %>%
  summarise(mean = round(mean(C), 1))

mdt$Date <- as.Date(as.POSIXct(mdt$Date, format = "%m/%d/%Y"))

# Select the columns you need from sid
sid2 <- sid %>% select(Date, jdate)
sid2$Date <- as.Date(as.POSIXct(sid2$Date, format = "%m/%d/%Y"))

# merge with sid
mdt <- merge(sid2, mdt)

# Rename mean
names(mdt)[3] <- 'Temperature'
...
# Get rid of jdate
t.f.p.fl <- t.f.p.fl %>% select(-(jdate))
...
write.csv(t.f.p.fl, "C:/Users/mschraml/Desktop/ROutput/BOR Daily Passage and FL UBC.csv", row.names = FALSE)
