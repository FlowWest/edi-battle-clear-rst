...
# Create a data frame for the biweekly passage and confidence limits, change brood years if needed
biweekly <- data.frame(matrix(NA, nrow = 3, ncol = 8))
names(biweekly) <- c("Biweekly rub", "Winter-run Chinook Salmon 21", "Winter-run Chinook Salmon 22", "Spring-run Chinook Salmon 21",
 "Late-fall run Chinook Salmon 21", "Late-fall run Chinook Salmon 22",
"RainbowTrout/steelhead 21", "RainbowTrout/steelhead 22")

biweekly [1,1] <- "90% LCL"
biweekly [2,1] <- "Passage index"
biweekly [3,1] <- "90% UCL"

# Create a data frame for the brood year passage and confidence limits, change brood years if needed
brood.year <- data.frame(matrix(NA, nrow = 3, ncol = 8))
names(brood.year) <- c("Biweekly rub", "Winter-run Chinook Salmon 21", "Winter-run Chinook Salmon 22", "Spring-run Chinook Salmon 21",
                       "Late-fall run Chinook Salmon 21", "Late-fall run Chinook Salmon 22",
                       "RainbowTrout/steelhead 21", "RainbowTrout/steelhead 22")

brood.year [1,1] <- "90% LCL"
brood.year [2,1] <- "Passage index"
brood.year [3,1] <- "90% UCL"

# Select for only UBC data
catch <- catch %>% filter(StationCode == "UBC")
...
for (i in 1:nrrbt.1) {
  ...
  rbt.int.1[i, 2] <- round(lcl.90)                    # Lower 90% confidence limit
...
biweekly[1, 7] <- rbt.biweekly.passage.1          # biweekly passage
biweekly[2, 7] <- round(rbt.Alcl.90a.1, 0)        # biweekly Lower 90% confidence limit
biweekly[3, 7] <- round(rbt.Aucl.90a.1, 0)        # biweekly Upper 90% confidence limit
...
############## Add data to biweekly Table ##############
biweekly[1, 8] <- rbt.biweekly.passage.2          # biweekly passage
biweekly[2, 8] <- round(rbt.Alcl.90a.2, 0)        # biweekly Lower 90% confidence limit
biweekly[3, 8] <- round(rbt.Aucl.90a.2, 0)        # biweekly Upper 90% confidence limit
...
############## Add data to biweekly Table ##############
biweekly[1, 5] <- lfcs.biweekly.passage.1          # biweekly passage
biweekly[2, 5] <- round(lfcs.Alcl.90a.1, 0)        # biweekly Lower 90% confidence limit
biweekly[3, 5] <- round(lfcs.Aucl.90a.1, 0)        # biweekly Upper 90% confidence limit
...
############## Add data to biweekly Table ##############
biweekly[1, 6] <- lfcs.biweekly.passage.2          # biweekly passage
biweekly[2, 6] <- round(lfcs.Alcl.90a.2, 0)        # biweekly Lower 90% confidence limit
biweekly[3, 6] <- round(lfcs.Aucl.90a.2, 0)        # biweekly Upper 90% confidence limit
...
# Calculate the biwwekly passage Index
...
biweekly[2, 2] <- round(wcs.Alcl.90a.1, 0)        # biweekly Lower 95% confidence limit
biweekly[3, 2] <- round(wcs.Aucl.90a.1, 0)        # biweekly Upper 95% confidence limit
...
biweekly[2, 3] <- round(wcs.Alcl.90a.2, 0)        # biweekly Lower 95% confidence limit  # biweekly passage Index
biweekly[3, 3] <- round(wcs.Aucl.90a.2, 0)        # biweekly Upper 95% confidence limit
...
# Create a new a data frame with only the data below
scs.eff <- select(scs.2, strata.week, BaileysEff, NumReleased)
...
# Group data by strata
scs.2 <- group_by(scs.2, strata.week)

# Summarize catch data by strata 
scs.2 <- summarise(scs.2, 
                    w.catch = sum(RCatch))

# Order catch data by strata
scs.2 <- arrange(scs.2, strata.week)

# Combine efficiency and catch data sets
scs.2 <- left_join(scs.2, scs.strataeff, by = c('strata.week'))
...
############## Calculate the actual biweekly passage and the total passage index ##############
...
############## Add data to biweekly Table ##############
biweekly[1, 4] <- scs.biweekly.passage          # biweekly Lower passage
...
brood.year[1, 7] <- rbt.brood.passage.d             # brood year passage
brood.year[2, 7] <- round(rbt.Alcl.90a.d, 0)        # brood year Lower 95% confidence limit
brood.year[3, 7] <- round(rbt.Aucl.90a.d, 0)        # brood year Upper 95% confidence limit
...
############## Add Rainbow Trout/steelhead first brood year data to brood year table Table BY ##############
brood.year[1, 8] <- rbt.brood.passage.e             # brood year passage
brood.year[2, 8] <- round(rbt.Alcl.90a.e, 0)        # brood year Lower 95% confidence limit
brood.year[3, 8] <- round(rbt.Aucl.90a.e, 0)        # brood year Upper 95% confidence limit
...
brood.year[1, 5] <- lfcs.brood.passage.d          # brood year passage
brood.year[2, 5] <- round(lfcs.Alcl.90a.d, 0)        # brood year Lower 95% confidence limit
brood.year[3, 5] <- round(lfcs.Aucl.90a.d, 0)        # brood year Upper 95% confidence limit
...
# Calculate brood year passage Index
...
############## Add Late-fall data to brood year Table ##############
brood.year[1, 6] <- lfcs.brood.passage.e             # brood year passage
brood.year[2, 6] <- round(lfcs.Alcl.90a.e, 0)        # brood year Lower 90% confidence limit
brood.year[3, 6] <- round(lfcs.Aucl.90a.e, 0)        # brood year Upper 90% confidence limit
...
# Calculate the brrood year passage Index
...
brood.year[2, 2] <- round(wcs.Alcl.90a.d, 0)        # brood year Lower 95% confidence limit
brood.year[3, 2] <- round(wcs.Aucl.90a.d, 0)        # brood year Upper 95% confidence limit
...
# Create a new a data frame with only the data below
scs.eff <- select(scs.e, strata.week, BaileysEff, NumReleased)
...
# Combine efficiency and catch data sets
scs.e <- left_join(scs.e, scs.strataeff, by = c('strata.week'))
...
brood.year[2, 4] <- round(scs.Alcl.90a1, 0)        # brood year Lower 95% confidence limit
brood.year[3, 4] <- round(scs.Aucl.90a1, 0)        # brood year Upper 95% confidence limit
...
write.csv(l, "C:/Users/mschraml/Desktop/ROutput/UBC Bootstraps.csv", row.names = FALSE)
