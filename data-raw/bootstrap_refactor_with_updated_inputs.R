...
catch_data <- catch |> 
  ...
  # filter(brood_year %in% brood_years,
  #        week %in% subset_weeks) |> 
...
library(boot)
get_passage <- function(data, indices) {
  data_subset <- data[indices, ]
  recaptures <- round(rbinom(1, data_subset$number_released, data_subset$efficiency))
  bootstrap_passage <- round(data_subset$weekly_catch * (data_subset$number_released + 1) / (recaptures + 1), 0)
  return(bootstrap_passage)
}

test <- boot(data = strata_catch_summary |> 
               filter(!is.na(efficiency)) |> 
               group_by(common_name, fws_run, brood_year, station_code), 
             statistic = get_passage,
             R = 1000)
boot.ci(test, type = "bca")

set.seed(2323)
bootstrapping <- strata_catch_summary |> 
  filter(!is.na(efficiency)) |> 
  group_by(common_name, fws_run, brood_year, station_code) |> 
  group_split() |> 
  purrr::map(function(x) {
    if(length(x$common_name)<= 1) {
      return(tibble(common_name = unique(x$common_name),
                    fws_run = unique(x$fws_run),
                    #strata = unique(x$strata),
                    station_code = unique(x$station_code),
                    brood_year = unique(x$brood_year),
                    lcl = NA,
                    weekly_passage = median(x$weekly_passage),
                    ucl = NA))
    } else {
      bs <- boot(data = x, 
                 statistic = get_passage,
      ...
      lcl <-CIs[length(CIs)-1]
      ucl <- CIs[length(CIs)]
      
      return_tibble <- tibble(common_name = unique(x$common_name),
                              fws_run = unique(x$fws_run),
                              #strata = unique(x$strata),
                              station_code = unique(x$station_code),
                              brood_year = unique(x$brood_year),
...
# TODO when returning weekly passage it needs to be: 
